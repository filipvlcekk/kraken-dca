...
Currently configured project: `web-ui-cron-scheduler` (target: `..`).
...
3. Open the parent repo in your coding agent (Claude Code, Cursor, …)
   and have it follow `data/web-ui-cron-scheduler/SETUP.md` to fill in
   `data/web-ui-cron-scheduler/INFO.md`.
...
`--project-id web-ui-cron-scheduler` (or whichever id you want) explicitly.

`scan` is free (regex only). `process` is the AI stage (≈$0.30/file
on Opus by default). Run state goes to `data/web-ui-cron-scheduler/`.
